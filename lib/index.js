...
function visitImport(node) {
  ...
  var before = handleLineno(node.lineno) + '@use ';
...
function visitCall(_ref4) {
  ...
  var ruptureScaleNamesSingleQ = ['\'xxs\'', '\'xs\'', '\'s\'', '\'m\'', '\'l\'', '\'xl\'', '\'xxl\''];
  var ruptureScaleNamesDoubleQ = ['\"xxs\"', '\"xs\"', '\"s\"', '\"m\"', '\"l\"', '\"xl\"', '\"xxl\"'];
...
function visitBinOp(_ref7) {
  ...
  if (typeof visitNode(leftExp) === 'string' && visitNode(leftExp).includes('%s')) {
    var variable = /\d+[(px)(em)%]/.test(expText) ? '' + expText : '#{' + expText + '}';
